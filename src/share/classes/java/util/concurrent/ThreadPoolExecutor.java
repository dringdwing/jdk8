...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /*
     * 不需要解压缩 ctl 的位域访问器。这些取决于 bit 布局和 workerCount 永远不会为负数。
     */
    ...
    final void runWorker(Worker w) {
        ...
        w.firstTask = null;
        // 新创建的Worker默认state为-1，AQS的unlock方法会将其改为0，此后允许使用interruptIfStarted()方法进行中断
        w.unlock();
    ...
    /**
     * 当我们向线程池提交一个任务时 无论是通过 execute、submit还是
     * invokeAll/invokerAny 方法最终都会走到 execute 方法上来
     *
     * 主要逻辑和机制
     * 核心线程处理: 优先保证核心线程数量，确保有足够的线程处理任务。
     * 队列机制: 当核心线程满时，将任务放入队列，延迟处理，防止过多线程创建。
     * 动态检查: 在任务入队后重新检查线程池状态，以处理可能的变化（如线程池关闭或线程死亡）。
     * 拒绝策略: 当任务无法执行时（例如线程池满或已关闭），使用拒绝策略处理任务
     */
    ...
    /**
     * 线程池通过 mainLock 来保证全局配置的线程安全，而每个工作线程再通过 AQS 来保证工作线程自己的线程安全。
     */
